...
t_final = 60
# r = Rotation.from_rotvec(-np.pi/9 * np.array([1, 0, 0]))
...
for i in range(len(q_des)):
    angs_des = Rotation.from_quat(q_des[i]).as_euler('zxy',degrees=True)
    Phi_des.append(angs_des[1])
    angs_st = Rotation.from_quat(q[i]).as_euler('zxy', degrees=True)
    Phi_st.append(angs_st[1])
    Phi_step.append(0)
Phi_step[0] = x[0,0]
...
#######################
(fig, axes) = plt.subplots(nrows=1, ncols=1, sharex=True, num='Roll Angle vs Time')
ax = axes

ax.plot(time, Phi_step, 'r')
ax.plot(time, Phi_st, 'r.')
ax.legend(('roll angle'))
ax.set_ylabel('roll angle (deg)')
ax.set_xlabel('time, s')
ax.grid('major')

(fig, axes) = plt.subplots(nrows=1, ncols=1, sharex=True, num='x-step vs Time')
ax = axes

ax.plot(time, x[:,0], 'r')
ax.plot(time, Phi_step, 'b')
ax.set_ylabel('x (m)')
ax.set_xlabel('time, s')
ax.grid('major')
